...
def mettre_a_jour_pip():
    subprocess.run([sys.executable,"-m","pip","install",
        "--upgrade","pip","--quiet"], capture_output=True, timeout=30)
...
class SplashScreen(tk.Tk):
    ...
    def _build(self):
        tk.Label(self, text="Imprimante", font=("Segoe UI",36),
            bg="#003087", fg="white").pack(pady=(20,5))
        tk.Label(self, text="Brother Scan Center",
            font=("Segoe UI",18,"bold"),
            bg="#003087", fg="white").pack()
        self.card = tk.Frame(self, bg="#f0f4f8", padx=15, pady=10)
        self.card.pack(fill="x", padx=25, pady=15)
        self.status = tk.Label(self.card, text="Initialisation...",
            font=("Segoe UI",10,"bold"), bg="#f0f4f8", fg="#003087")
        self.status.pack(anchor="w")
        self.detail = tk.Label(self.card, text="",
        ...
        self.log = tk.Text(self.card, height=7,
            font=("Consolas",8), bg="#1e1e1e", fg="#00ff00",
            relief="flat", state="disabled")
        self.log.pack(fill="x")
        tk.Label(self, text="v1.0.0",
            font=("Segoe UI",8), bg="#003087", fg="#6c9fd4"echo             ).pack(side="bottom", pady=5)
    ...
    def _verifier(self):
        ...
        self.bar["value"] = 100
        if erreurs:
            self.set_status("Erreurs d installation",
                ", ".join(erreurs))
